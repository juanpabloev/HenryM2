...
createdBy.innerHTML = createdBy.innerHTML + " Juan Evangelista";
...
function buildToDos(toDos) {
  // Tu código acá:
  let index = 0;
  const todoMaps = toDos.map(function (todo) {
    let ar = buildToDo(todo, index);
    index++;
    return ar;
  });
  return todoMaps;
...
// Tu código acá:
let agregarButton = document.querySelector("#addButton");
agregarButton.addEventListener("click", addToDo);
